...
pub const NAME_VIEW_IMPLEMENTS : &str  = "implements";
// Hash of the name of the view "implements" described in TIP-100.
const HNAME_VIEW_IMPLEMENTS : ScHname  =  generator::generate_schname!("implements");
...
pub fn implements(hname_contract : ScHname, hname_interface : ScHname, ctx : &ScViewContext) -> bool {
    ...
    let result = ctx.call(hname_contract, HNAME_VIEW_IMPLEMENTS, Some(input_params));
    let implements = results::get_bool(NAME_VIEW_IMPLEMENTS, result);
    implements
}
